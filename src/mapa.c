...
char symbol_equipos[N_EQUIPOS] ={'A', 'B', 'C', 'D'/*, 'E'*/};
...
char mapa_get_ganador(tipo_mapa *mapa)
{
	int j, flag = 0;
	int winner;
	...
	if(flag == 1) {
		tipo_nave nave = mapa_get_nave(mapa, winner, 0);
		return mapa_get_symbol(mapa, nave.posy, nave.posx);
	}
